...
export class MinePowerupsFarPathfinding {
	constructor(main) {
		this.main = main;
	...
	pathfind(fromX, fromY, myRange, invulnerable) {
		...
		this.antiGoalX = this.main.player.intPos.x;
		this.antiGoalY = this.main.player.intPos.y;
		this.gLimit= 10;
	...
	mark(x, y, g, exploreList) {
		...
		const h = Math.abs(x - this.antiGoalX) + Math.abs(y - this.antiGoalY);
		if (inRange > 0 && h > this.goalValue && dt === 0) {
			this.goalValue = h;
			this.goalX = x;
			this.goalY = y;
		}
